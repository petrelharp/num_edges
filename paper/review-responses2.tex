...
We appreciate the additional comments by the reviewers, and thank you for the acceptance into Genetics.
...
\reply{
	Thank you for your comment. We have added the assumption of the parent node and child node having unique times in the previous section where the notation for parent and child nodes are introduced in the previous section, \revref. We have additionally included your re-phrasing to describe the uniqueness between the sets $\{u_i\}$ and $\{v_j\}$.
}

\begin{point}{\revref}
	P. 3 around line 21 says we are extending nodes ${u_i}$ to $T_{k+1}$. This sounds like it
	modifies $T_{k+1}$ by adding unary nodes to it. Around line 25 says that we are modifying
	$T_k$. p. 4 says we are modifying both $T_k$ and $T_{k+1}$. Please clarify.
\end{point}

\reply{
	Thank you for bringing this to our attention. Algorithm 2 as stated in page 4, line 27-29 can alter both trees $T_k$ and $T_{k+1}$ which is why they are updated at each transition between $T_k\to T_{k+1}$. We have added this point on \revref to note that both trees can be modified during any iteration of Algorithm 2.
}
...
\begin{point}{\revref}
	p. 9 "here we require" tripped me up: does it refer to the method used throughout
...
\begin{point}{\revref}
	p. 9 lines 54-55. You simulated "ARGs modified from a 'true' ARG"--modified how?
	The text doesn't seem to say at all.
\end{point}

\reply{
	Thanks for the comment. We perform the modifications explained in the Results subsections \textit{Accuracy with true trees} and \textit{Inferred ARGs}. We refer to Figure 6 in this section for clarity on the modifications.
}
