...
The ancestor and descendant are represented by the parent node $p_e$ and child node $c_e$ respectively \revpoint{2}{1}
...
\begin{algorithm2e}[!ht]
    ...
    \caption{
        ...
        for which edges $O_k$ are removed to transition from $T_k$ to $T_{k+1}$,
...
\begin{algorithm2e}[!ht]
    ...
    \Fn{\mergeable($c$, $T_O$, $T_I$)}{
        ...
        \While{\tn{True}}{
            ...
            \eIf{$y_i$}{
                \If{$P_I[c] \neq p_i$ and $P_O[c] \neq p_i$}{
                    \text{Set } $m_e = m_e + 1$. \tcc*[r]{$p_i$ is not in $T_O$}
                }
                \text{Set } $c = p_i$,
                $p_i = P_I[p_i]$, \text{ and}\;
                ...
                \If{$P_I[c] \neq p_o$ and $P_O[c] \neq p_o$}{
                    \text{Set } $m_e = m_e + 1$. \tcc*[r]{$p_o$ is not in $T_I$}
                }
                \text{Set }$c = p_o$,
                $p_o = P_O[p_o]$, \;
                $t_o = \text{ if }(p_o = NULL)\text{ then }\infty\text{ else }t[p_o]$ ,\;
                \text{and } $m = m + 1$ . \tcc*[r]{$p_o$ would be extended into $T_I$.}
            }
        }
        \If{$m = 0$ or $p_i \neq p_o$ or $p_i = NULL$}{
...
an entirely different set of quantities.) \llabel{asymmetric}
...
$d_{\varphi_{12}}(\T_1,\T_2) + d_{\varphi_{23}}(\T_2,\T_3) \le d_{\varphi_{12}\circ\varphi_{23}}(\T_1,\T_3)$.

This differs from the definitions above because here
...
and second, this definition makes computation much more difficult.
...
shows that much of the span removed by simplify has been replaced --
...
than is span after simpmlifying (points in C).
...
It may be possible to better deal with the latter situation,
