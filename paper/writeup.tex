...
Each edge $e \in E$ describes inheritance between a parent node $p_e$ and child node $c_e$, \revpoint{2}{1}
...
so that $u_1=3$ (and $m=1)$ and there are no $v$ (so $n=0$); \revpoint{3}{11}
...
The method is implemented in the \tscompare{} package.
...
\includegraphics[height=1.5in, width=3in]{figures/discrepancy_function_method_t2.pdf}
    \caption{
        ...
        Node $4$ has no match on $[4,5)$ and matches with node $9$ on $[5,6)$. Thus the maximal mapping for 4 should be to itself. 
        On the rest of the genome, all of nodes match with their identical counterpart.
...
the ``simplified'' line (``S'') on the bottom two plots of Figure~\ref{fig:dissimilarity} shows
