...
\begin{figure}
    \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/conceptual_figure.pdf}
    \end{center}
    \caption{
...
and let $\|\T_1\|=\sum_{n\in N_1}s(\T_1,n)$ be the total sum of nodes spans for nodes in $\T_1$.
...
Contrarily, given a matching $\beta:\T_1\to\T_2$, 
...
Note that the inverse matched span is not equivalent
 to the matched span of $\T_2$ in $\T_1$ 
 as the inverse matched span maximizes over spans
  which are still dependent on $\beta$, 
  and, as previously stated, $\beta$ is not guaranteed to be bijective.
...
We compute the ARF and TPR of two tree sequences in \figref{fig:conceptual_discrepancy}.
...
Neither the matched span or non-matched span of $\T_1$ in $\T_2$ are not metrics in the mathematical sense
...
(Simplification does not produce non-matched span.)
...
\begin{figure}[!hbt]
	\begin{center}
        \includegraphics[width=\textwidth]{figures/figure6-ARF-TPR-plt.pdf}
	\end{center}
    \caption{
...
by around 2\% (bottom panels),
but that roughly 20\% of the unary spans in the inferred ARG are incorrect (top panels).
Furthermore, comparing to the inferred-extend(``IE''; red) line, we see that extending the tree sequence output with tsinfer adds relatively little span ($\approx <1\%$).
...
along a single path \citep{ralph2013geography}.
Largely for computational reasons, this is the definition that is used in \tskit's
IBD-finding methods 
(by G.~Tsambos \citet{tskit}; see also \citet{tsambos2023link}).
