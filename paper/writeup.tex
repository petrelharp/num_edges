...
\begin{abstract}
    ...
    and contain substantially more information
...
for each node, the dark blue point is the total span in a ``true'' tree sequence
...
The green point is the span of the node after \emph{simplifying} the tree sequence
...
The implementation of this method, called \emph{tree\_dissimilarity} \comment{change?}, in \texttt{tsdate}
produces three values after computation: 
the relative dissimilarity, weighted root-mean-square-error, and true proportion represented. 
The \emph{relative dissimilarity} is defined to be the dissimilarity proportional
 to the total span of nodes in $\T_1$,
 \begin{align}\label{eqn:dissimilarity}
 	\overline{\dis}(\T_1,\T_2)=\frac{\dis(\T_1,\T_2)}{\sum_{n\in N_1} s(\T_1,n)}
 	=1-\frac{\similarity(\T_1,\T_2)}{\sum_{n\in N_1} s(\T_1,n)}.
\end{align}
The \emph{weighted root-mean-square-error} is given in \eqnref{eqn:wrmse}.
The \emph{true proportion represented} is similarity between two trees relative 
to the total span of nodes in $\T_2$,
\begin{align}\label{eqn:true-proportion}
	\overline{\similarity}(\T_1,\T_2) = \frac{\similarity(\T_1,\T_2)}{\sum_{n\in N_2}s(\T_2,n)}.
\end{align}
...
We compute the tree\_dissimilarity of two tree sequences in \figref{fig:conceptual_discrepancy}.
...
\includegraphics[height=1.5in, width=3in]{discrepancy_function_method_t2.pdf}
    \caption{
        ...
        This makes the dissimilarity $\dis(T1,T2)=3$ and the \emph{relative similarity} $\overline{\dis}(T1,T2)=\frac{3}{46}$.
...
All jobs used for recording runtime were run on an Intel Xeon Gold 6148 processor.
To observe the accuracy of our method, we compute node spans for a correctly and 
incorrectly added for a tree sequence simulated with
 1,000 samples over a genome of length $10^8$.
 Additionally, we compare accuracy between sequences modified from a ``true'' tree sequence
 using our dissimilarity method. 
 These tree sequences range from 10 to 1,000 samples with genome length $5\times 10^7$
 as well as having 1,000 samples and ranging the genome length from $10^6$ to $5\times 10^7$.
All jobs used for recording node span and (dis)similarity for true and inferred ARGs
 were run on an Intel Core i5-1035G4 processor.
...
\begin{figure}[bht]
	...
	\caption{
        ...
        in an ARG simulated with $10^4$ \comment{diploid?} samples on a sequence of length $10^8$.
        \textbf{(A)}
...
\figref{fig:dissimilarity} shows in purple total amounts of span removed by simplification
and re-inferred by \algorithmref{alg:extend} (correctly and incorrectly).
(Lines in green involve re-inference of the ARG; discussed next.)
The top row shows the proportion of the given ARG that does not match the original,
...
The bottom row shows the proprtional of the original ARG that is represented in the given ARG.
This shows that a large proportion of the ARG can be removed by simplifiction,
...
then re-inferred ARGs from their genotypes and performed various operations on the results.

First, \figref{fig:dissimilarity} shows that \tsinfer{}
has a substantial portion of already--extended haplotypes:
comparing the ``inferred'' (``I''; dotted) line to the ``inferred-simplified'' (``IS''; dashed) line
...
\comment{Also say something about IE line...}
...
We began work on this paper with the simple observation that the simple transformation
...
and that contain more accurate information about true ancestral relationships.
These extended ancestral haplotypes manifest as unary nodes in the marginal trees.
Although a number of ARG inference methods may be taking advantage of this information,
it is our impression that this source of information is not widely appreciated in the community.
In fact, the field's focus on marginal trees rather than haplotypes
meant that to study accuracy of this method,
we had to invent new haplotype-aware measures of (dis)agreement between ARGs.
...
removing this with simplification decreases both dissimilarity
(i.e., proportion of ``wrong'' haplotypes)
and the proportion of the truth that is correctly inferred.
...
of the ARG of \figref[A\&B]{fig:conceptual} would be that we do not know
...
say $|N_1|<|N_2|$ we can take matching maps $\varphi\colon\T_1\to\T_2$ (not surjective)
and $\psi\colon T_2\to T_1$ (not injective)
and compute the dissimilarity as symmetric difference between $|\T_1\setminus \T_2|_{\varphi}|$
and $|\T_2\setminus \T_1|_\psi|$. This should still be a metric \comment{?????}
then \comment{???} .
