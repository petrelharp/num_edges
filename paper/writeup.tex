...
The \emph{ancestral recombination graph robinson-foulds} (ARF) is defined to be the dissimilarity proportional
...
This may at first be somewhat surprising:
This makes sense because
as described in \citet{ralph2020efficiently},
each edge added or removed requires an update
to the state of the parent node and nodes above it,
and extending haplotypes yields a tree sequence with fewer edge removals and insertions. 
...
The bottom row shows the proprtional of the original ARG that is represented in the given ARG (TRP).
...
removing this with simplification decreases both ARF
...
Singer [cite] returns ARGs without any unary nodes,
although these are reconstructable using the times of the recombination nodes.
On the other hand, locally building trees separately in each segment of genome
