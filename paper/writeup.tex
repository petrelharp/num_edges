...
at each position of the genome \citep{XXX}.
...
This is reflected in methodology of some ARG inference methods \citep[e.g.,][]{Relate},
...
For instance, \citet{XXX} compared true and inferred ARGs
using average XXX distance between trees across a regular sequence of genomic positions,
...
which suggest that \comment{XXX TODO}.
...
that was simulated with \texttt{msprime} \citep{msprime}
...
Other authors \citep{XXX} have used
...
perhaps using the Subgraph-Prune-and-Regraft moves of \citet{singer}.
...
The ``simplification'' algorithm of \citet{XXX},
...
\begin{figure}
	\includegraphics[width=0.9\linewidth]{newplots_wo_ee/num_trees_vs_Num_edges_subplot.pdf}
    \caption{
        Raw numbers of edges before and after extending haplotypes
        for the tree sequences shown in \figref{fig:num_edges}
        \label{sfig:num_edges}
    }
\end{figure}
