...
The ancestor and descendant are represented, respectively, by the parent node $p_e$ and child node $c_e$ of the edge and occur at unique times so that $t_{p_e} \neq t_{c_e}$. \revpoint{2}{1}\revpoint{3}{1}
...
so that $u_1=3$ (and $m=1)$ and there are no $v$ (so $n=0$); \revpoint{3}{11}\revpoint{3}{12}
...
(which includes possibly modifying both $T_k$ and $T_{k+1}$) as it goes.\revpoint{3}{2}
...
the removed edges that make up $F_O$ \llabel{ref_conceptual2}
...
(i.e., the next older node, as recall $t_n$ is amount of time in the past that $n$ lived) \llabel{time_reminder}
is in $F_I$ or $F_O$.
...
Note that the algorithm could be applied to an undated ARG, \llabel{undated}
...
in practice we do repeated passes in both directions until no changes can be made. \revpoint{2}{2}
...
Additionally, using our matched span methods, we compare accuracy between sequences modified from a ``true'' ARG, see \figref{fig:dissimilarity}.\revpoint{3}{5} 
