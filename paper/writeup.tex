...
% redefining \input{} messes things up (and seems like a bad idea)
...
\begin{abstract}
    ...
    In a genealogical context, this recombination-mediated information reflects the persistence
    of ancestral haplotypes across local genealogical trees in which they
    do not represent coalescences.
    We show how these non-coalescing haplotypes (``locally-unary nodes'') may be
    ...
    We provide efficient algorithms to infer locally-unary nodes within existing ARGs,
...
and the connection to parsimony. \revpoint{1}{7}
...
to take advantage of the % extensive, stable, and well-documented set of open-source 
...
which is the amount of time in the past that the individual who carried that segment of genome lived. \revpoint{3}{13}
...
The ancestor and descendant are represented, respectively, by the parent node $p_e$ and child node $c_e$ of the edge and occur at unique times so that $t_{p_e} > t_{c_e}$. \llabel{unique_times}
...
and the times of the nodes $\{u_i\}$ and $\{v_j\}$ are unique, \revpoint{3}{1}
meaning no node in ${u_i}$ has the same time as a node in ${v_j}$.
...
and then extend the nodes $\{u_i\}$ to $T_{k+1}$. \revpoint{3}{2}
...
(which includes possibly modifying $T_{k+1}$) as it goes. \llabel{extend_k}
...
Suppose an edge in $I$ has child $c$.
...
\algorithmref{alg:extend} simplifies the full algorithm implemented in software % in \tskit{}
...
% however it is not something we thoroughly investigated in the theoretical setting.
...
Furthermore, the forests $F_O$ and $F_I$ (and hence $T_{k+1}$)
need to be updated. \llabel{extend_k2}
...
\begin{algorithm2e}[!ht]
    ...
    \caption{
        Extend haplotypes. Given an ARG $\T$ with $N$ trees $T_1, \ldots, T_N$
        for which edges $I_k$ are added to transition from $T_k$ to $T_{k+1}$,
        identify and merge all mergeable paths (see text).
        Each child node $c_e$ of each removed edge $e$ is checked to see if it
        is at the base of two mergeable paths;
        paths that add fewer new edges are merged first
        because \texttt{MergeNum} returns the number of new edges required
        to merge the two paths, and $M$ is always less than or equal to
        the minimum number of new edges across all nodes in the current tree transition.
    }
	\label{alg:extend}
	\SetKwFunction{mergeable}{MergeNum}
	\SetKwFunction{merge}{Merge}
	\SetKwFunction{extend}{ExtendHaplotypes}
	\DontPrintSemicolon
    \Fn{\extend($\T$)}{
    	\tcc{For pairs of trees in the ARG, find mergeable paths.}
        \For{$k$ in $1 \ldots N-1$}{
            \text{Set } $M = 0$ \text{ and }
            $M' = \infty$.\;
            \While{$M < \infty$}{
                \For{$e \in I_k$}{
                    \text{Set } $m_e$ = \mergeable($c_e$, $T_k$, $T_{k+1}$).\tcc*[r]{Find the number of new edges required.}
                    \eIf{$m_e \le M$}{
                        \merge($c_e$, $T_k$, $T_{k+1}$)\tcc*[r]{Extended haplotypes from $T_k$ into $T_{k+1}$.}
                    }{
                        \text{Set } $M' = \min(m_e, M')$.
                    }
                }
...
\begin{algorithm2e}[!ht]
    ...
    \caption{
        ...
        (i.e., in $F_O$ and $F_I$);
        these are NULL if $n$ has no parent,
        and we take $t[\text{NULL}] = \infty$.
        The variable $m_e$ will record the number of new edges to be added,
        and $m$ will record the number of extended haplotypes.
        Note that if no haplotypes can be extended ($m=0$), we return $m_e$ as $\infty$
        so that the path is not merged in \algorithmref{alg:extend}.
    }
	\label{alg:mergeable}
	\SetKwFunction{mergeable}{MergeNum}
	\DontPrintSemicolon
    \Fn{\mergeable($c$, $T_O$, $T_I$)}{
    	 %\tcc{We construct paths beginning at $c$ and traverse through inheritance in $T_O$ and $T_I$.}
        \text{Let } $p_i = P_I[c]$, 
        $t_i = t[p_i]$,
        $p_o = P_O[c]$,
        $t_o = t[p_o]$,
        ...
        \While{\tn{True}}{
            \text{Set } $y_i = (p_i \neq NULL)
                \ \&\ (p_i \notin T_O)
                \ \&\ (t_i < t_o)$ \tcc*{Next node is from $T_I$.}
            \text{and } $y_o = (p_o \neq NULL)
                \ \&\ (p_o \notin T_I)
                \ \&\ (t_o < t_i)$ \tcc*{Next node is from $T_O$.}
            \If{not ($y_i$ or $y_o$)}{ \Break\; }
            \eIf{$y_i$}{
                \If{$P_I[c] \neq p_i$}{
                    \text{Set } $m_e = m_e + 1$. \tcc*[r]{$c \to p_i$ is a new edge.}
                }
                \text{Set } $c = p_i$,
                $p_i = P_I[p_i]$, \text{ and }
                $t_i = t[p_i]$ .\;
                ...
                \If{$P_O[c] \neq p_o$}{
                    \text{Set } $m_e = m_e + 1$. \tcc*[r]{$c \to p_o$ is a new edge.}
                }
                \text{Set }$c = p_o$,
                $p_o = P_O[p_o]$,
                $t_o = t[p_o]$ \;
                % $t_o = \text{ if }(p_o = NULL)\text{ then }\infty\text{ else }t[p_o]$ ,\;
                \text{and } $m = m + 1$ . \tcc*[r]{$c \to p_o$ would be extended.}
            }
        }
        \If{$m = 0$ or $p_i \neq p_o$ or $p_i = NULL$}{
            \text{Set } $m_e = \infty$. \tcc*[r]{The paths cannot be merged.}
        }
        \KwRet{$m_e$}\;
    }
\end{algorithm2e}
...
The method (including \eqnref{eqn:matched-spans} - \eqnref{eqn:true-proportion}) \revpoint{1}{3}
...
in what follows we think of $\T_2$ as the true ARG and $\T_1$ as an inferred ARG. \llabel{truth}
...
a different set of quantities.)\revpoint{1}{1} \llabel{asymmetric}
...
whose best match is $n_2$.  \revpoint{3}{15}
...
Then the average RF distance (averaged over locations in the genome) \revpoint{1}{2}
is
\begin{align*}
...
The ARG RF (which we call ``ARF'') is defined to be the non-matched span proportional
...
We give an example computing the ARF and TPR between two small tree sequences in \figref{fig:conceptual_discrepancy}. \revpoint{3}{3}
...
\paragraph{Metrics on ARGs} \revpoint{1}{5}
...
This metric, $\min_\varphi d_\varphi(\T_1,\T_2)$, differs from the definitions above because
...
that $\alpha$ matches to the same node in $\T_2$). \revpoint{3}{4}
We did not require this property when defining non-matched span above for two reasons:
...
% Additionally, as \textit{ARG Robinson-Foulds} is given as a proportion of node spans
...
Hudson's algorithm as implemented in \msprime{} \revpoint{2}{3}
...
Additionally, using our matched span methods, we compare accuracy between sequences modified from a ``true'' ARG,
(see \figref{fig:dissimilarity}). \revpoint{3}{5} 
...
We next present the results of various experiments with inferred ARGs. \revpoint{3}{6}
First, we quantify the reduction in number of edges and resulting computational speedups.
Next, to validate the principle of the algorithm,
we describe accuracy of inferred additional edge spans when applied to true ARGs with unary spans removed.
Finally, we explore implications for accuracy with inferred ARGs.
...
95\% of nodes have no incorrectly added span; \revpoint{2}{4}
...
by around 50\%.
For instance, \figref[A]{fig:node-spans} \revpoint{3}{19}
...
at the cost of a more complex algorithm. \llabel{bad_spans}
...
The bottom row shows the proportion of the original ARG that is represented in the given ARG (``TPR'').
...
Presumably, this occurs because both correctly and incorrectly inferred haplotypes are extended. \revpoint{1}{6}
...
Another consideration is storage and computational efficiency: \llabel{inferred_runtime}
...
\paragraph{Ignorance and omission in an ARG} \llabel{ignorance}
...
\begin{table}[!h]
	\begin{center}
		\begin{tabular}{{|c||c|c|c|c|c||c|c|c|c|c|}}
			\hline
			& \multicolumn{5}{|c||}{ARF} & \multicolumn{5}{|c|}{TPR}\\
			\hline
			\hline
