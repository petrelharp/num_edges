...
\begin{algorithm2e}[!ht]
    ...
    \caption{
        ...
        because \texttt{Mergeable} returns the number of new edges required
    ...
    \Fn{\extend($\T$)}{
        ...
        \For{$k$ in $1 \ldots N$}{
            ...
            \While{$M < \infty$}{
                \For{$e \in O_k$}{
                    \text{Set } $m_e$ = \mergeable($c_e$, $T_k$, $T_{k+1}$).\tcc*[r]{Count the number of new edges to be merged.}
                    \eIf{$m_e \le M$}{
                        \merge($c_e$, $T_k$, $T_{k+1}$)\tcc*[r]{Merge extended haplotypes from $T_k$ into $T_{k+1}$.}
                    }{
                        \text{Set } $M' = \min(m_e, M)$.\tcc*[r]{Bookkeep $M$ to determine an ordering for path merging.}
                    }
                }
                \text{Set } $M = M'$ \text{ and }
                $M' = \infty$ .\;
...
\begin{algorithm2e}[!ht]
    ...
    \Fn{\mergeable($c$, $T_O$, $T_I$)}{
        ...
        \text{ and }$m_e = m = 0$.\;
        \While{\tn{True}}{
        	\tcc{States to determine if paths can still be constructed backward in time.}
            \text{Set } $y_i = (p_i \neq NULL)
                \ \&\ (p_i \notin T_O)
                \ \&\ (t_i < t_o)$ \;
            \text{and } $y_o = (p_o \neq NULL)
            ...
            \eIf{$y_i$}{
                \If{$P_I[c] \neq p_i$ and $P_O[c] \neq p_i$}{
                    \text{Set } $m_e = m_e + 1$. \tcc*[r]{$p_i$ is not in $T_O$ so we add a new edge.}
                }
                \text{Set } $c = p_i$,
                $p_i = P_I[p_i]$, \text{ and}\;
                ...
                \If{$P_I[c] \neq p_o$ and $P_O[c] \neq p_o$}{
                    \text{Set } $m_e = m_e + 1$. \tcc*[r]{$p_o$ is not in $T_I$ so we add a new edge.}
                }
                \text{Set }$c = p_o$,
                $p_o = P_O[p_o]$, \;
                $t_o = \text{ if }(p_o = NULL)\text{ then }\infty\text{ else }t[p_o]$ ,\;
                \text{and } $m = m + 1$ . \tcc*[r]{We extend haplotype from $p_o$ into $T_I$.}
            }
        }
        \If{$m = 0$ or $p_i \neq p_o$ or $p_i = NULL$}{
            \text{Set } $m_e = \infty$. \tcc*[r]{The paths constructed at $c$ cannot be merged.}
        }
        \KwRet{$m_e$}\;
    }
\end{algorithm2e}
...
We compute the ARF and TPR of in a simple example in \figref{fig:conceptual_discrepancy}.\revpoint{1}{4}

\revpoint{3}{16}\revpoint{3}{17}
\begin{figure}[!ht]
	\begin{center}
...
by around 50\% (\figref[C, D]{fig:node-spans}).\revpoint{3}{19}
