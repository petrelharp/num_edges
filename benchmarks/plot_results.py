import sys, os
import json
import glob
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import matplotlib

assert len(sys.argv) == 2, "Usage: python plot_results.py <csv file>"
infile = sys.argv[1]

df = pd.read_csv(infile)
basename = ".".join(infile.split(".")[:-1])

length_vals = np.unique(df.length)
length_cols = plt.colormaps["viridis"](np.linspace(0, 1, len(length_vals)))
ns_vals = np.unique(df.num_samples)
ns_cols = plt.colormaps["viridis"](np.linspace(0, 1, len(ns_vals)))

# combined number of edges & runtime for Tajimas D
fname = f"{basename}_absolute_values.pdf"
fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(9, 5))

ax0.set_xlabel("sequence length")
ax1.set_xlabel("sequence length")
ax0.set_ylabel("number of edges")
ax1.set_ylabel("runtime, Tajima's D")
for ns, this_df in df.groupby("num_samples"):
    sub_df = this_df.sort_values("length")
    j,  = np.where(ns == ns_vals)
    ax0.plot(sub_df.length, sub_df.num_edges_before, marker=".",
             c=ns_cols[j])
    ax0.plot(sub_df.length, sub_df.num_edges_after, marker="x",
             c=ns_cols[j])
    ax1.plot(sub_df.length, sub_df.runtime_before, marker=".", label=f"samples={ns}",
             c=ns_cols[j])
    ax1.plot(sub_df.length, sub_df.runtime_after, marker="x",
             c=ns_cols[j])

ax1.legend()

plt.tight_layout()
plt.savefig(fname)


# combined number of edges & runtime for Tajimas D
fname = f"{basename}_ratios.pdf"
fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(9, 5))

ax0.set_xlabel("sequence length")
ax1.set_xlabel("sequence length")
ax0.set_ylabel("number of edges, after/before")
ax1.set_ylabel("runtime, after/before")
ax0.set_ylim((0, 1))
ax1.set_ylim((0, 1))
for ns, this_df in df.groupby("num_samples"):
    sub_df = this_df.sort_values("length")
    j,  = np.where(ns == ns_vals)
    ax0.plot(sub_df.length, sub_df.num_edges_after / sub_df.num_edges_before,
             marker=".", label=f"samples={ns}",
             c=ns_cols[j])
    ax1.plot(sub_df.length, sub_df.runtime_after / sub_df.runtime_before,
             marker=".", label=f"samples={ns}",
             c=ns_cols[j])

ax0.legend()

plt.tight_layout()
plt.savefig(fname)


# timing plots
fname = f"{basename}_timing.pdf"
fig, (ax0, ax1, ax2) = plt.subplots(1, 3, figsize=(9, 4), sharey=True)

ax0.set_xlabel("number of samples")
ax0.set_ylabel("runtime (s)")
ax1.set_xlabel("sequence length")
ax1.set_ylabel("runtime (s)")
ax2.set_xlabel("number of edges")
ax2.set_ylabel("runtime (s)")
ax0.set_yscale("log")
ax1.set_yscale("log")
ax2.set_yscale("log")
for L, this_df in df.groupby("length"):
    sub_df = this_df.sort_values("num_samples")
    j,  = np.where(L == length_vals)
    ax0.plot(sub_df.num_samples, sub_df.extend_time, marker="o", label=f"length={L:g}",
             c=length_cols[j])

for ns, this_df in df.groupby("num_samples"):
    sub_df = this_df.sort_values("length")
    j,  = np.where(ns == ns_vals)
    ax1.plot(sub_df.length, sub_df.extend_time, marker="o", label=f"samples={ns}",
             c=ns_cols[j])

for L, this_df in df.groupby("length"):
    sub_df = this_df.sort_values("num_samples")
    j,  = np.where(L == length_vals)
    ax2.scatter(sub_df.num_edges_before, sub_df.extend_time, marker="o", label=f"length={L:g}",
                c=ns_cols[j])

ax0.legend()
ax1.legend()
ax2.legend()

plt.tight_layout()
plt.savefig(fname)
